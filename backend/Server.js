...
app.get(`/api/Search`, (req, res) => {
  const favTweetUrl = `https://api.twitter.com/2/users/${req.query.searchTweet}/tweets?tweet.fields=public_metrics,author_id&user.fields=profile_image_url&expansions=author_id`;
  const config = {
    headers: { Authorization: `Bearer ${token}` },
  };
  axios
